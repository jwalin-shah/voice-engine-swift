#!/usr/bin/env python3
"""
Export Moonshine-base encoder + stateful decoder to CoreML .mlpackage.

Requirements:
    pip install coremltools transformers torch numpy sentencepiece

Output:
    ~/.cache/moonshine-coreml/base/
        encoder.mlpackage
...
# Architecture constants — derived from HuggingFace config.
# base: hidden_size=416, heads=8, head_dim=52, layers=8, partial_rotary_factor=0.62
H      = 8    # num_attention_heads
HID    = 416  # hidden_size
D      = 52   # head_dim = HID // H
NL     = 8    # num_decoder_layers
ROT_DIM = 32  # int(D * partial_rotary_factor) = int(52 * 0.62) = 32

S_MAX     = 128   # max self-attention positions
S_ENC_MAX = 500   # max encoder frames
...
def extract_kv_weights():
    ...
    out = {
        ...
        "cos_tables": np.stack(cos_tables),
        "sin_tables": np.stack(sin_tables),
    }
    for i in range(NL):
...
def export_id_to_piece():
    """Export id_to_piece.json for Swift tokenizer use."""
    print("→ Exporting id_to_piece.json…")
    from transformers import AutoTokenizer
    tok = AutoTokenizer.from_pretrained(MODEL_NAME)
    vocab = tok.get_vocab()
    # get_vocab() returns {token: id} — reverse to {id: token}
    payload = {"id_to_piece": {str(v): k for k, v in vocab.items()}}
    path = OUTPUT_DIR / "id_to_piece.json"
